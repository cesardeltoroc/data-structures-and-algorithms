...
const longestString = (arr) => {
  let longStr = {string: '', longestIndex: -1};
  arr.forEach((str, index) => {
    str.length > longStr.string.length ? longStr = {string: str, longestIndex: index} : '';
  });
  return longStr.longestIndex;
};
...
const firstLetters = (arr) => {
  let ans = arr.map(str => str.charAt(0));
  return ans;
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(newNum => `${newNum.substring(1, 4)}${newNum.substring(6, 9)}${newNum.substring(10)}`);
};
...
const allHappy = (arr) => {
  return arr.every(idx => idx.includes(':)'));
};
