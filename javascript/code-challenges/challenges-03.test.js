...
const typeNum = (arr) => {
  ...
  let numbers = arr.filter(numOnly);
  return numbers;
};
...
const containsAnd = (arr) => {
  var charc = arr.filter(charInc => charInc.includes('and'));
  return charc;
};
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter((curArr) => !forbiddenValues.includes(curArr));
